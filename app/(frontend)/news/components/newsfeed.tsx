...
export default function NewsFeed({
  ...
  return (
    <div className="flex flex-row">
      ...
      <div className="max-w-full w-4/6 mb-2 border-0 rounded-xs bg-[#DDA520]/80 text-xl px-2 pt-2">
        {" "}
        {/*ori color: AF8219*/}
        {/*Beige card - Image and Texts*/}
        <div
          className={`${isExpanded ? "" : "max-h-70"} bg-[#E6E6E6]/60 text-[#302F2F] flex`}
        >
          {" "}
          {/*ori color: DCCDAA*/}
          {/*News Image*/}
          ...
          <div className="w-[42.5%] m-[1.5%] flex flex-col justify-center">
            <p className="font-dm font-medium text-xs"> ▶ {news.subtitle}</p>{" "}
            {/*font is not font-ing :(((*/}
            <p className="font-[23] text-2xl"> {news.title}</p>
            <div className={isExpanded ? "" : "max-h-25 overflow-hidden"}>
              <RichText
                data={news.description}
                className={`font-[Fira_Mono] text-sm ${isExpanded ? "" : "line-clamp-3"}`}
              />{" "}
              {/*font is not font-ing :(((*/}
            </div>
            {/*Categories*/}
            <div className="flex flex-wrap justify-left mt-2">
              {categories.map((cat) => (
                <div
                  key={cat.id}
                  className="bg-black text-[#E2E2E2] m-2 py-1 px-[7.5%] text-sm text-center font-[Fira_Mono]"
                >
                  {cat.name}
                </div> /*font is not font-ing :(((*/
              ))}
            </div>
          </div>
        </div>
        ...
        >
          <div className="max-w-full flex items-center">
            <div className="bg-[#DDA520] border-2 border-[#FEFEFE] rounded-full w-7 h-7 m-2 flex justify-center-safe items-center-safe">
              {" "}
              {/* check margin and press tab in the rendering its kinda bad ngl*/}
      ...
      <div className="flex flex-col relative mb-2 w-1/6 ">
        ...
        <div className="absolute top-1/2 -ml-13 left-0 h-fit w-4/5 -translate-y-1/2 border border-[#DDA520] bg-black rotate-270">
          <div className="text-[#DDA520] text-2xl text-center">
