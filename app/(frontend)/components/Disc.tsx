...
export default function Disc({ name, url, image, icon, iconWidth, iconHeight, iconRight, iconTop, color }: DiscProps) {
    return (
        <Link href={url} target="_blank">
            <div className={`relative w-65 h-75 ${color} bg-opacity-10 rounded-xl overflow-hidden`}>
                <Image src={image} width={192} height={224} alt={name} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full -mr-2" />
